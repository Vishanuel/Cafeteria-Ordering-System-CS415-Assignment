    ...
    <div class="box">
        ...
        <div class="box-body ">
            <table class="table table-bordered table-striped text-center" id="example1">
                ...
                @for($i=0;$i<count($ingredients);$i++) <tr>
                    <td>
                    ...
                    <div class="modal fade" id="{{$ingredients[$i]->Item_Ingredient_ID}}" data-backdrop="static"
                        data-keyboard="false">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form method="post"
                                    action="{{ route('ingredient.update', $ingredients[$i]->Item_Ingredient_ID) }}"
                                    ...
                                    <div class="modal-body">
                                        <div class="form-group">
                                            ...
                                            <select id="menus" name="menu_item" class="form-control">
                                                @for($j=0;$j<count($menu);$j++) <option
                                                    value={{$menu[$j]->Menu_Food_Item_ID}} @if($menu[$j]->
                                                    Menu_Food_Item_ID == $ingredients[$i]->Item_ID)
                                                    selected="selected"@endif>{{$menu[$j]->Food_Name}}</option>
                                                    @endfor
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="ingredient_name"
                                                value="{{$ingredients[$i]->Ingredient_Name}}" required>
                                        </div>

                                        @for($k=0;$k<count($type);$k++) <div>
                                            <input type="radio" name="type" value="{{$type[$k]->Ingredient_Type_ID}}"
                                                @if(($ingredients[$i]->Ingredient_Type_ID)==($type[$k]->Ingredient_Type_ID))
                                            ? checked : @endif>
        ...
        <div class="modal fade" id="delete{{$ingredients[$i]->Item_Ingredient_ID}}">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form method="post" action="{{ route('ingredient.destroy',$ingredients[$i]->Item_Ingredient_ID) }}"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    ...
                    <form method="post" action="{{ route('ingredient.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <select id="menus" name="menu_item" class="form-control">
                                    <option value=""></option>
                                    @for($j=0;$j<count($menu);$j++) <option value="{{$menu[$j]->Menu_Food_Item_ID}}">
                                        {{$menu[$j]->Food_Name}}</option>
                                        @endfor
                                </select>
                            </div>
                            <div class="form-group">
