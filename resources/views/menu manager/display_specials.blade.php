...
<section class="content">
    <div class="box">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped" id="example1">
                    ...
                    @for($i=0;$i<count($data);$i++) <tr>
                        ...
                        @for($k=0;$k<1;$k++) <td>
                            @for($j=0;$j<count($val[$i]);$j++) <div>{{$val[$i][$j]->Food_Name}}
            </div>
            @endfor
            </td>
            <td>
            ...
            <div class="modal fade" id="edit{{$data[$i]->Special_ID}}" data-backdrop="static" data-keyboard="false">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form method="post" action="{{ route('specialmenu.update', $data[$i]->Special_ID) }}"
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    ...
                                    <select id="menus" name="menu" class="form-control">
                                        <option value=""></option>
                                        @for($j=0;$j<count($menu);$j++) <option value="{{$menu[$j]->Menu_ID}}"
                                            @if($menu[$j]->Menu_ID == $data[$i]->Menu_ID) selected="selected"@endif>
                                            {{$menu[$j]->Menu_Date}}
                                            @for($k=0;$k<count($menuitem[$j]);$k++) - {{$menuitem[$j][$k]->Food_Name}}
                                                @endfor - {{$menu[$j]->Category_Name}} </option> @endfor </select>
                                                </div> <div class="form-group">
                                                <label>Special Menu Description</label>
                                                <input type="text" class="form-control" name="menu_desc"
                                                    value="{{ $data[$i]->Special_Desc}}">
                                </div>
                                <div class="form-group">
                                    <label>Special Menu Price</label>
                                    <input type="text" class="form-control" name="menu_price"
                                        value="{{ $data[$i]->Special_Price}}">
                                </div>

                                <label>Select Menu Items for Special Menu </label>
                                @for($k=0;$k<count($items);$k++) <div>
                                    <input type="checkbox" name="items[]" value="{{$items[$k]->Menu_Food_Item_ID}}"
                                        @for($n=0;$n<count($val[$i]);$n++)
                                        @if(($val[$i][$n]->Menu_Food_Item_ID)==($items[$k]->Menu_Food_Item_ID)) ?
                                    checked :
                                    @endif @endfor>
        ...
        <div class="modal fade" id="delete{{$data[$i]->Special_ID}}">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form method="post" action="{{ route('specialmenu.destroy', $data[$i]->Special_ID) }}"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <select id="menus" name="menu" class="form-control">
                                    <option value=""></option>
                                    @for($j=0;$j<count($menu);$j++) <option value="{{$menu[$j]->Menu_ID}}">
                                        <div>{{$menu[$j]->Menu_Date}}
                                            @for($k=0;$k<count($menuitem[$j]);$k++) - {{$menuitem[$j][$k]->Food_Name}}
                                                @endfor - {{$menu[$j]->Category_Name}}</div> </option> @endfor </select>
                                                </div> <div class="form-group">
                                                <label>Special Menu Description</label>
                                                <input type="text" class="form-control" name="menu_desc">
                                        </div>
                                        ...
                                        @for($k=0;$k<count($items);$k++) <div>
                                            <input type="checkbox" name="items[]"
                                                value="{{$items[$k]->Menu_Food_Item_ID}}">
                                            <label for="type">{{$items[$k]->Food_Name}}</label>
                            </div>
                            @endfor
